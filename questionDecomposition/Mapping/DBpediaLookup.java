...
import Mapping.EntityFragmentFields;
...
public class DBpediaLookup {
	...
	public ArrayList<EntityMapping> getEntityMappings(String searchString) 
	{
		ArrayList<String> slist = new ArrayList<String>();
		if(entMentionDict.containsKey(searchString))
			slist.add(entMentionDict.get(searchString));
		else
			slist = lookForEntityNames(searchString);
		
		if (slist.size() == 0 && searchString.contains(". "))		
			slist.addAll(lookForEntityNames(searchString.replaceAll(". ", ".")));		
	...
	public ArrayList<String> lookForEntityNames (String searchString) {
		// URL transition: " " -> %20
		...
		try {
			while (true) {
				...
				try {
					...
					ArrayList<String> res = dbplook.lookForEntityNames(searchString);
