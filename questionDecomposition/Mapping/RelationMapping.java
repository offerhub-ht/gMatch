...
public class RelationMapping {
    ...
    public RelationMapping() {
        this.serverIP = RelationMapping.defaultServerIP;
        this.serverPort = RelationMapping.defaultServerPort;
    }

    public RelationMapping(int _port) {
        this.serverIP = RelationMapping.defaultServerIP;
        this.serverPort = _port;
    }

    public RelationMapping(String _ip, int _port) {
        this.serverIP = _ip;
        this.serverPort = _port;
    }
...
//NOTICE: no need to connect now, HTTP connection is kept by default
    public boolean load(String _db_name, String _username, String _password) {
		...
		if (!connect_return) {
			System.err.println("connect to server error. @RelationMapping.load");
			return false;
		}
        ...
        //if (!send_return) {
            //System.err.println("send load command error. @RelationMapping.load");
            //return false;
        //}
    ...
    public boolean unload(String _db_name,String _username, String _password) {
        boolean connect_return = this.connect();
        if (!connect_return) {
            System.err.println("connect to server error. @RelationMapping.unload");
            return false;
        }
    ...
    public boolean build(String _db_name, String _rdf_file_path, String _username, String _password) {
        boolean connect_return = this.connect();
        if (!connect_return) {
            System.err.println("connect to server error. @RelationMapping.build");
            return false;
        }
    ...
    public boolean drop(String _db_name) {
        boolean connect_return = this.connect();
        if (!connect_return) {
            System.err.println("connect to server error. @RelationMapping.drop");
            return false;
        }
    ...
    public String query(String _username, String _password, String _db_name, String _sparql) {
        boolean connect_return = this.connect();
        if (!connect_return) {
            System.err.println("connect to server error. @RelationMapping.query");
 ...
 //   public String show() {
  ...
  //  }
    ...
    public String show() {
        boolean connect_return = this.connect();
        if (!connect_return) {
            System.err.println("connect to server error. @RelationMapping.show");
            return "connect to server error.";
        }
    ...
    private static byte[] packageMsgData(String _msg) {
        ...
        byte[] head = RelationMapping.intToByte4(context_len);
        for (int i = 0; i < 4; i++) {
            data[i] = head[i];
        }
    ...
    public static void main(String[] args) {
        ...
        RelationMapping gc = new RelationMapping("dbpedia16.gstore-pku.com", 80);

        // build a new database by a RDF file.
        // note that the relative path is related to gserver
        //gc.build("db_LUBM10", "example/rdf_triple/LUBM_10_GStore.n3");
        String sparql1 = "SELECT distinct ?s WHERE {   ?s  <alumnus> ?o .  }";

//        boolean flag = gc.load("dbpedia16", "root", "123456");
        //System.out.println(flag);
        String answer = gc.query("root", "123456", "dbpedia16", sparql1);
